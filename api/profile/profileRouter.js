...
router.get('/fetching/budget/:profileId',(req,res)=>{
  ...
  Profiles.findById(profileId).then((profile) => {
    ...
    axios.get(`http://saverlife-c.eba-swb5qwdy.us-east-1.elasticbeanstalk.com/dev/forecast/statement/?user_id=${dsBody.dsUserId}&end_year=${dsBody.endYear}&end_month=${dsBody.endMonth}&goal=${dsBody.goal}`)
    .then(budget=>{
      let inputBody = {
